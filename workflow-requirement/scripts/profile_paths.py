#!/usr/bin/env python3
"""Resolve common project paths from profile configuration for workflow-requirement.
...
class ProjectPaths:
    ...
    def from_profile(cls, profile: dict[str, Any], workspace_root: Path | str) -> ProjectPaths:
        ...
        configured_task_board = _resolve_configured_file(root, docs_root, paths_cfg.get("task_board"))
        task_board = configured_task_board or _resolve_existing_file(
            root,
            None,
            (
        ...
        readme_index = configured_readme_index or _resolve_existing_file(
            root,
            None,
            ("doc/README.md", "docs/README.md", "README.md"),
            fallback_name="README.md",
        )
        configured_tasks_index = _resolve_configured_file(root, docs_root, paths_cfg.get("tasks_index"))
        tasks_index = configured_tasks_index or _resolve_existing_file(
            root,
            None,
...
def _resolve_configured_file(root: Path, docs_root: Path, configured: str | None) -> Path | None:
    if not configured:
        return None
